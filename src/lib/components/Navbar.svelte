<script>
	let navbarOpen = false;
	import { fade, fly } from 'svelte/transition';
	import { cubicInOut } from 'svelte/easing';

	const toggleNavbar = () => {
		navbarOpen = !navbarOpen;
	};
</script>

<section class="overflow-hidden" data-sveltekit-prefetch>
	<div class="flex items-center justify-between px-7 xl:px-40 py-5 lg:py-7 bg-white">
		<div class="w-auto">
			<div class="flex flex-wrap items-center">
				<div class="w-auto mr-14">
					<a href="/">
						<span
							class="mt-2 block text-2xl text-center leading-8 font-normal tracking-tight text-gray-900"
							>Amal Shaji</span
						>
					</a>
				</div>
			</div>
		</div>
		<div class="w-auto">
			<div class="flex flex-wrap items-center">
				<div class="w-auto hidden lg:block">
					<ul class="flex items-center mr-16">
						<li class="mr-9 font-medium text-lg hover:text-gray-700">
							<a href="/blog">Blog</a>
						</li>
						<li class="mr-9 font-medium text-lg hover:text-gray-700">
							<a href="/projects">Projects</a>
						</li>
						<li class="mr-9 font-medium hover:text-gray-700">
							<a
								href="https://github.com/amalshaji"
								target="_blank"
								rel="noreferrer"
								aria-label="github profile url"
								><svg
									xmlns="http://www.w3.org/2000/svg"
									class="icon icon-tabler icon-tabler-brand-github"
									width="24"
									height="24"
									viewBox="0 0 24 24"
									stroke-width="2"
									stroke="currentColor"
									fill="none"
									stroke-linecap="round"
									stroke-linejoin="round"
								>
									<path stroke="none" d="M0 0h24v24H0z" fill="none" />
									<path
										d="M9 19c-4.3 1.4 -4.3 -2.5 -6 -3m12 5v-3.5c0 -1 .1 -1.4 -.5 -2c2.8 -.3 5.5 -1.4 5.5 -6a4.6 4.6 0 0 0 -1.3 -3.2a4.2 4.2 0 0 0 -.1 -3.2s-1.1 -.3 -3.5 1.3a12.3 12.3 0 0 0 -6.2 0c-2.4 -1.6 -3.5 -1.3 -3.5 -1.3a4.2 4.2 0 0 0 -.1 3.2a4.6 4.6 0 0 0 -1.3 3.2c0 4.6 2.7 5.7 5.5 6c-.6 .6 -.6 1.2 -.5 2v3.5"
									/>
								</svg></a
							>
						</li>
						<li class="font-medium hover:text-gray-700">
							<a
								href="https://twitter.com/amal_ytics"
								target="_blank"
								rel="noreferrer"
								aria-label="twitter profile url"
								><svg
									xmlns="http://www.w3.org/2000/svg"
									class="icon icon-tabler icon-tabler-brand-twitter"
									width="24"
									height="24"
									viewBox="0 0 24 24"
									stroke-width="2"
									stroke="currentColor"
									fill="none"
									stroke-linecap="round"
									stroke-linejoin="round"
								>
									<path stroke="none" d="M0 0h24v24H0z" fill="none" />
									<path
										d="M22 4.01c-1 .49 -1.98 .689 -3 .99c-1.121 -1.265 -2.783 -1.335 -4.38 -.737s-2.643 2.06 -2.62 3.737v1c-3.245 .083 -6.135 -1.395 -8 -4c0 0 -4.182 7.433 4 11c-1.872 1.247 -3.739 2.088 -6 2c3.308 1.803 6.913 2.423 10.034 1.517c3.58 -1.04 6.522 -3.723 7.651 -7.742a13.84 13.84 0 0 0 .497 -3.753c-.002 -.249 1.51 -2.772 1.818 -4.013z"
									/>
								</svg></a
							>
						</li>
					</ul>
				</div>
				<div class="w-auto lg:hidden">
					<button on:click={toggleNavbar} aria-label="navbar open button">
						<svg
							class="navbar-burger text-white border border-gray-600 rounded-full"
							width="51"
							height="51"
							viewBox="0 0 56 56"
							fill="none"
							xmlns="http://www.w3.org/2000/svg"
						>
							<rect width="56" height="56" rx="28" fill="currentColor" />
							<path
								d="M37 32H19M37 24H19"
								stroke="black"
								stroke-width="1.5"
								stroke-linecap="round"
								stroke-linejoin="round"
							/>
						</svg>
					</button>
				</div>
			</div>
		</div>
	</div>
	{#if navbarOpen}
		<div class="navbar-menu fixed top-0 left-0 bottom-0 w-4/6 sm:max-w-xs z-50">
			<!-- svelte-ignore a11y-click-events-have-key-events -->
			<div
				transition:fade
				on:click={toggleNavbar}
				class="navbar-backdrop fixed inset-0 bg-gray-500 opacity-80"
			/>
			<nav
				transition:fly={{ x: -200, duration: 200, easing: cubicInOut }}
				class="relative z-10 px-9 pt-8 bg-white h-full overflow-y-auto"
			>
				<div class="flex flex-wrap justify-between h-full">
					<div class="w-full">
						<div class="flex items-center justify-between -m-2">
							<div class="w-auto p-2">
								<a class="inline-block" href="/">
									<span
										class="mt-2 block text-2xl text-center leading-8 font-normal tracking-tight text-gray-900"
										>Amal Shaji</span
									>
								</a>
							</div>
							<div class="w-auto p-2">
								<button
									class="navbar-burger"
									on:click={toggleNavbar}
									aria-label="navbar open button"
								>
									<svg
										width="24"
										height="24"
										viewBox="0 0 24 24"
										fill="none"
										xmlns="http://www.w3.org/2000/svg"
									>
										<path
											d="M6 18L18 6M6 6L18 18"
											stroke="#111827"
											stroke-width="2"
											stroke-linecap="round"
											stroke-linejoin="round"
										/>
									</svg>
								</button>
							</div>
						</div>
					</div>
					<div class="flex flex-col justify-center py-16 w-full">
						<ul>
							<li class="mb-12">
								<a class="font-medium hover:text-gray-700" href="/blog">Blog</a>
							</li>
							<li class="mb-12">
								<a class="font-medium hover:text-gray-700" href="/projects">Projects</a>
							</li>
							<li class="mb-12">
								<a
									class="font-medium hover:text-gray-700"
									href="https://github.com/amalshaji"
									target="_blank"
									rel="noreferrer">GitHub</a
								>
							</li>
							<li>
								<a
									class="font-medium hover:text-gray-700"
									href="https://twitter.com/amal_ytics"
									target="_blank"
									rel="noreferrer">Twitter</a
								>
							</li>
						</ul>
					</div>
				</div>
			</nav>
		</div>
	{/if}
</section>
