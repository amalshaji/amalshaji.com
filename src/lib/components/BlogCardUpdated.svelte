...
<div class="w-full md:w-1/2 px-4 mb-8">
	<a
		class="block mb-6 overflow-hidden rounded-md"
		href={`/blog/${post.path.replace('.md', '').replace('.svx', '').replace('/blog/', '/')}`}
	>
		<img class="w-full" src={post.metadata.image} alt={post.metadata.title} />
	</a>
	<!-- <div class="mb-4">
		<a
	...
	<p class="mb-2 text-coolGray-500 font-medium"><Date date={post.metadata.date} /></p>
	<a
